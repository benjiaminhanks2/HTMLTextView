...
import be.vrt.ui.utils.findAppropriateViewType
...
class AdapterDelegatesManager<T : AbsHTMLElementAdapterDelegate<RecyclerView.ViewHolder>> {
    ...
    fun getItemViewType(items: List<HTMLElement>, position: Int): Int =
            delegates.findAppropriateViewType(FALLBACK_DELEGATE_VIEW_TYPE) {
                it.isForViewType(items[position], items, position)
            }
